...
public class DBdataForReceivedInv {
    ...
    public ObservableList<Invoices> querySelectWithParam(String whereColumn, String whereEquals) {
        String query = "SELECT * from erp_items.receivedinvoices WHERE " + whereColumn + "=" + "?";
        Connection conn = DBConnection.getInstance().getConnection();
        try (PreparedStatement ps = conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public ObservableList<Invoices> querySelectWithParam(String whereColumn, int whereFrom, int whereTo) {
        String query = "SELECT * from erp_items.receivedinvoices WHERE " + whereColumn + " BETWEEN ? AND ?";
        Connection conn = DBConnection.getInstance().getConnection();
        try (PreparedStatement ps = conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public void queryInsert(Invoices invoice) {
        String query = "INSERT INTO erp_items.receivedinvoices (invoiceNumberProp, dateOfReceiptProp, totalAmountProp, amountDueProp, " +
                "dueDateProp, supplierProp, subjectProp, projectNumberProp, projectManagerProp, noteProp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        Connection conn = DBConnection.getInstance().getConnection();
        try (PreparedStatement ps = conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {;
    ...
    public void queryUpdate(Invoices invoice) {
        ...
        String query =
                "UPDATE erp_items.receivedinvoices SET " +
                        "invoiceNumberProp=?, " +
                        "dateOfReceiptProp=?, " +
                        "totalAmountProp=?, " +
                        "amountDueProp=?, " +
    ...
    public void queryDelete(Invoices invoice) {
        int item_id = invoice.getId();
        String query = "DELETE FROM erp_items.receivedinvoices WHERE id=?";
    ...
    private ObservableList<Invoices> prepareData (ResultSet resultSet) {
        ...
        try {
            while (resultSet.next()) {
                ...
                Date date = resultSet.getDate("dateOfReceiptProp");
                ...
                String recipient = resultSet.getString("supplierProp");
